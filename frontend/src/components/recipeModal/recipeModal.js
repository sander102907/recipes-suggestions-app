...
const RecipeModal = (props) => {
  ...
  const addIngredientToGroup = async (ingredient, group_id) => {
    await axios
      .get(`/api/ingredients/ah/${ingredient.ah_id}`, { params: ingredient })
      .then((response) => {
        axios.post("api/groups/ingredient", {
          group_id: group_id,
  ...
  const addGroupToRecipe = (recipe_id) => {
    return axios
      .post("/api/groups", { recipe_id: recipe_id })
  ...
  const addIngredient = (suggestion) => {
    ...
    const ingredient = {
      ...
      image_tiny: suggestion.images[0].url.replace(
        "200x200_JPG.JPG",
        "48x48_GIF.GIF"
      ),
  ...
  const createRecipe = (recipe) => {
    axios.post("/api/recipes", recipe).then((response) => {
      ...
      groups.forEach((group) => {
        promises.push(
          addGroupToRecipe(response.data.id).then((group_id) => {
  ...
  const updateRecipe = (recipe) => {
    axios.put(`/api/recipes/${props.id}`, recipe).then(() => {
      axios.delete(`/api/groups/recipe/${props.id}`).then(() => {
        const promises = [];

        groups.forEach((group) => {
          promises.push(
            addGroupToRecipe(props.id).then((group_id) => {
              group.ingredients.forEach(async (ingredient) => {
                await addIngredientToGroup(ingredient, group_id);
              });
            })
      ...
      <Modal.Body>
        ...
        <ReactQuill
          ...
          onBlur={(previousRange, source, editor) =>
            setDescriptionLength(editor.getLength())
          }
